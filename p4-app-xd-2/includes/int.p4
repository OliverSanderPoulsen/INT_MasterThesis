                    ...
                    in    standard_metadata_t standard_metadata)
{
    ...
    action int_add_ingress_timestamp () {
        meta.int_metadata.ingress_tstamp =
            (bit<32>) standard_metadata.ingress_global_timestamp;
    }

    // Modified to add int ingress timestamp.
    // table name have not been changed due to the controller using the same name as identification (Should be changed later)
    table tb_set_first_hop {
        key = {
            standard_metadata.ingress_port: exact;
        }
        actions = {
            int_add_ingress_timestamp;
        }
    }
                        ...
                        in    standard_metadata_t standard_metadata)
{
        ...
        bit<4> ins_mask0003, bit<4> ins_mask0407, bit<4> ins_mask1215)
    {
        ...
        */
        ...
        // As int-md and int-xd is not supported in the same implementation, this is not critical as of now.
        hdr.int_meta.ver = 1;
        hdr.int_meta.rep = 0;
        hdr.int_meta.c = 0;
        hdr.int_meta.e = 0;
...
// Mostly just checks if the int meta stack has room for more int data
// Not nessacary in case of postcard..
